...
use Hexboy\WooCommerce\HttpClient\BasicAuth;
use Hexboy\WooCommerce\HttpClient\HttpClientException;
use Hexboy\WooCommerce\HttpClient\JWTAuth;
use Hexboy\WooCommerce\HttpClient\Options;
...
class HttpClient
{
     ...
     */
    public function __construct($url, $authParams, $options)
    {
        ...
        $this->username = $authParams['username'] ?? null;
        $this->password = $authParams['password'] ?? null;
        $this->token    = $authParams['token'] ?? null;
        $this->headers  = [];
    }
    ...
    {
        $api = $this->options->isWPAPI() ? $this->options->apiPrefix() : '/wc/';
    ...
    protected function authenticate()
    {
        ...
        if ( $this->options->basicAuth() ) {
            ...
            $this->headers = array_merge($this->headers, $basicAuth->getHeaders());
        } else {
            $jwtAuth    = new JWTAuth($this->token);
            $this->headers = array_merge($this->headers, $jwtAuth->getHeaders());
        }
    }
    ...
    protected function getRequestHeaders($sendData = false)
    {
        ...
        $this->headers['User-Agent'] = $this->options->userAgent() . '/' . WooCommerceClient::VERSION;
